...
static int vault_aes256(char* buffer, char* key, int enc_mode);

int vault_encrypt_aes256(char* buffer, char* key) {
	return vault_aes256(buffer, key, 1);
}

int vault_decrypt_aes256(char* buffer, char* key) {
	return vault_aes256(buffer, key, 0);
}
...
int vault_aes256(char* buffer, char* key, int enc_mode) {
	check_init();
	...
	printf("vault_aes256: AES256 key length = %zu, block length = %zu, given key length = %zu given text length = %zu\n", key_length, block_length, strlen(key), text_length);
	...
	srand(time(NULL)); /* Give the RNG a quick seed. */
	int32_t ctr_value = rand() % pow(2, 32); /* This should be OK.. We want to avoid using the same CTR value twice (often). At this rate, is is negligible as a risk. (Ask, though.) */

	err = gcry_cipher_setctr(hd, ctr_value);
